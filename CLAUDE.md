...
`.claude/rules/` holds the enforceable per-area rules — read the matching file before touching that area:

| Rule file | Applies to |
|---|---|
| `typescript-style.md` | all TypeScript (types, control flow, file organization) |
| `api-conventions.md` / `api-security.md` / `api-testing.md` | `apps/api` controllers, services, DAOs, tests |
| `database-entities.md` / `database-queries.md` | MikroORM entities and query code |
| `zod-schemas.md` | DTO and validation schemas |
| `cost-accounting.md` | anything touching token usage, pricing, or numeric aggregation |
| `sandbox-boundary.md` | sandbox-bridged agents and bridge frame parsing |
| `oauth-mcp.md` | OAuth credentials and MCP node wiring |
| `web-components.md` / `web-patterns.md` | `apps/web` UI and page structure |
...
pnpm check-types                      # tsc --noEmit across the workspace (CI gate)
...
```bash
pnpm run full-check                   # build → check-types → build:tests → lint:fix → test:unit → test:integration
```

`full-check` runs the integration suite too, so **Docker must be up** before invoking it (see the Docker note under Testing).
...
pnpm seed:run                         # Run the root DatabaseSeeder
```
...
├── contexts/            # React contexts (ProjectContext and friends)
...
│   ├── projects/        # Project list/switcher — projects scope graphs, threads, secrets
│   ├── repositories/    # Git repository management
│   ├── knowledge/       # Knowledge base editor
│   ├── memory/          # Agent-memory namespaces + entries browser
│   ├── oauth/           # OAuth connections page + per-node Authenticate flow
│   ├── github-app/      # GitHub App install / linking
│   ├── settings/        # Secrets and project settings
│   ├── storybook/       # Component-library source of truth (see rules below)
│   └── main/            # Dashboard
...
- **Real-time**: Socket.io streams for graph compilation/deployment status, node execution state, agent message tokens (including streaming), revision lifecycle events, thread mutations. Subscribe via hooks: `useWebSocket`, `useGraphWebSocketHandlers`.
...
- **Adding a new graph node type**: Backend defines template schema -> `pnpm --filter @geniro/web generate:api` -> update `graph-node-card.tsx` if special rendering needed -> add validation rules in `GraphValidationService.ts`.
...
Test files are co-located as `*.spec.{ts,tsx}` next to the source. Run `pnpm --filter @geniro/web test:unit <relative-path>` to target a single file (or no path for the whole web suite). The web `test:unit` script is wired into `pnpm full-check` and CI (`.github/workflows/ci.yaml`) automatically.
...
│   ├── seeders/              # MikroORM seeder classes
│   └── mikro-orm.config.ts
...
| `subagents` | `SubAgent` runtime + subagent definitions consumed by `agent-tools` and `graph-templates` |
| `system-agents` | Built-in agent definitions registered into the template registry at boot (`onModuleInit`) |
...
| `agent-memory` | Durable project-scoped agent memory — Postgres KV + Qdrant semantic recall (`memory_search`) |
| `instruction-blocks` | Reusable instruction blocks surfaced as graph-node templates |
| `threads` | Thread/message/checkpoint persistence |
| `thread-store` | Per-thread key/value store exposed to agent tools |
| `graph-templates` | Pluggable node template registry |
| `graph-resources` | Resource providers wired into graphs (e.g. `GithubResource`) |
| `runtime` | Docker-based isolated execution (Dockerode); also K8s / Daytona transports |
| `notifications` | Socket.IO WebSocket event broadcasting |
| `notification-handlers` | Gateways + handlers that persist and fan out agent/thread events |
| `knowledge` | Vector embeddings + semantic search (Qdrant) |
| `qdrant` | Qdrant client wrapper |
| `litellm` | LLM proxy integration |
| `openai` | OpenAI-compatible chat/embeddings client used across modules |
| `ai-suggestions` | LLM-backed suggestions over graphs and threads |
| `projects` | Project entity — the scoping boundary for graphs, threads, secrets, memory |
| `git-repositories` | GitHub repo management (Octokit) |
| `git-auth` | GitHub App auth: installation/token resolution, webhook signature + subscription registry |
| `oauth-credentials` | Per-project OAuth credentials for MCP providers — exchange, capability link, refresh watchdog |
| `secrets` | User-managed secret CRUD referenced by graph nodes |
| `secrets-store` | Backing store abstraction for secret values (OpenBao) |
| `webhooks` | Webhook sync-state + processed-event dedupe registry |
| `analytics` | Usage/cost rollups (`GET /analytics/overview`, `/analytics/by-graph`) |
| `user-preferences` | Per-user preference persistence |
| `system` | System endpoints (`GET /system/settings`, `/system/config`) |
| `cache` | Redis caching layer |
| `utils` | Shared helpers used across v1 modules (`shell.utils.ts`) |

### GitHub App integration (optional)

The GitHub App feature (`git-auth` module) is the authentication method for GitHub integration. It is **optional** — when not configured, GitHub operations that require authentication will fail.
...
- **Task queue**: BullMQ (Redis) for async work like revision processing and knowledge reindexing. Any DB-tracked job must provide repair-on-read (reset stale Pending/InProgress rows on the next user interaction) OR a periodic watchdog — startup-only recovery is insufficient for long-running pods.
...
- **Migrations**: Always `pnpm run migration:generate`. Never hand-write migration files. Run `pnpm migration:run` to apply pending migrations. **Known drift exception**: the generator consistently emits unrelated `runtime_instances` enum/CHECK-constraint changes alongside any intended migration (pre-existing mismatch between native-ENUM entity and CHECK-constraint dev schemas). When this happens, hand-edit the generated file to remove only the `runtime_instances` statements and keep the intended SQL.
