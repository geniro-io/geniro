...
- **Migrations**: Always `pnpm run migration:generate`. Never hand-write migration files. Run `pnpm migration:run` to apply pending migrations. **Known drift exception**: the generator consistently emits SQL unrelated to the intended entity change (pre-existing dev-schema mismatches). Known drift families include `runtime_instances` enum/CHECK-constraint changes AND `thread_store_entries` FK-drop + `threads.total_running_ms` default normalization — but treat ANY unrelated statement as drift, not only these named families. When this happens, hand-edit the generated file to keep ONLY the intended statements and drop the unrelated drift (in both `up()` and `down()`).
