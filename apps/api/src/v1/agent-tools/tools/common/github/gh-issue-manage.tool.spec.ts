...
} from './gh-issue-manage.tool';

describe('GhIssueManageTool', () => {
  let tool: GhIssueManageTool;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [GhIssueManageTool],
    }).compile();

    tool = module.get<GhIssueManageTool>(GhIssueManageTool);
  });

  describe('properties', () => {
    it('should have correct name', () => {
      expect(tool.name).toBe('gh_issue');
    });
  });
    ...
    it('accepts a valid list action', () => {
      expect(() =>
        tool.validate({
          owner: 'acme',
          repo: 'demo',
          action: GhIssueManageAction.List,
        }),
      ).not.toThrow();
    });

    it('rejects a create action without title', () => {
      expect(() =>
        tool.validate({
          owner: 'acme',
          repo: 'demo',
          action: GhIssueManageAction.Create,
        }),
      ).toThrow(/title/);
    });
  });
  ...
  describe('invoke — fail-closed token resolution', () => {
    // gh_issue performs WRITES (close/label/reopen/create), so a dropped
    // re-throw here is the highest-impact fail-open of the four — arrange a
    // write action to match that rationale (the resolveToken guard fires before
    // action dispatch, so the action is incidental to the fail-closed result).
    const args: GhIssueManageToolSchemaType = {
      owner: 'acme',
      repo: 'demo',
      action: GhIssueManageAction.Close,
      issue_number: 42,
    };
