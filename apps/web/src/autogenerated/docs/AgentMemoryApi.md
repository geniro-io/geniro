...
| [**searchMemoryEntries**](#searchmemoryentries)   | **GET** /api/v1/memory/search               |             |
...
> Array<AgentMemoryEntryDto> searchMemoryEntries()
...
const { status, data } = await apiInstance.searchMemoryEntries(query, limit);
```
