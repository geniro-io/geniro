...
export const IntegrationsPage = () => {
  ...
  const fetchData = useCallback(async () => {
    // Only fetch GitHub App installations when the App is configured.
  ...
  return (
    <div className="space-y-6">
      ...
      {!settingsLoading &&
        !settings.githubAppEnabled &&
        !settings.githubUserPatEnabled && (
          <Alert>
            <Info className="h-4 w-4" />
            <AlertTitle>GitHub integration is not available</AlertTitle>
            <AlertDescription>
              Neither the GitHub App nor the secrets store (required for
              personal access tokens) is configured. Contact your administrator
