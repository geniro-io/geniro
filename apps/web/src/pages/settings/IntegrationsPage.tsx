...
import { SystemSettingsResponseDtoGithubAuthModeEnum } from '../../autogenerated/api';
...
export const IntegrationsPage = () => {
  ...
  const fetchData = useCallback(async () => {
    // Only fetch GitHub App installations when the App is the active, installable
    // integration. In pat mode there are no installations to list.
    if (!settings.githubAppInstallable) {
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {!settingsLoading &&
        !settings.githubAppInstallable &&
        settings.githubAuthMode !==
          SystemSettingsResponseDtoGithubAuthModeEnum.Pat && (
          <Alert>
            <Info className="h-4 w-4" />
            <AlertTitle>GitHub App integration is not available</AlertTitle>
            <AlertDescription>
              Contact your administrator to enable this feature by configuring
      ...
      {!settingsLoading &&
        settings.githubAuthMode ===
          ...
          <Alert>
            <Info className="h-4 w-4" />
            <AlertTitle>
              GitHub is connected via a personal access token
            </AlertTitle>
            <AlertDescription>
              This deployment uses a configured personal access token
              (GITHUB_AUTH_MODE=pat) instead of the GitHub App, so the App
              install/authorize flow is not used. GitHub features are
              {settings.githubAvailable
                ? ' available.'
                : ' unavailable until GITHUB_PAT is configured.'}
            </AlertDescription>
          </Alert>
        )}
