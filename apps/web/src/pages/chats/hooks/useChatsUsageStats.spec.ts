...
afterEach(() => {
  // Unmount every rendered hook so its fetch/transition effects are torn down.
  // Without this, mounted hooks accumulate across the file and keep the worker
  // event loop alive, ending in a forced-kill teardown hang.
  cleanup();
  vi.clearAllMocks();
});
...
describe('useChatsUsageStats — context-usage gauge by template', () => {
  beforeEach(() => {
    (
      threadsApi.getThreadUsageStatistics as ReturnType<typeof vi.fn>
    ).mockResolvedValue(makeUsageStats({ currentContext: 50_000 }));
  });
  ...
  const makeGraph = (
    ...
    ({
      id: 'graph-1',
      schema: { nodes: [{ id: 'node-1', template, config }] },
    }) as unknown as GraphDto;

  const renderForNode = (graph: GraphDto) =>
    renderHook(() =>
      useChatsUsageStats({
        toastMessage,
        selectedThread: makeThread({ status: 'done' }),
        selectedThreadId: 'thread-1',
        selectedThreadIsDraft: false,
        selectedAgentNodeId: 'node-1',
        graphCache: {},
        getFullGraph: () => graph,
      }),
    );

  it('claude-agent: derives the 200K base window → 50K/200K = 25%', async () => {
    const { result } = renderForNode(
      makeGraph('claude-agent', { maxContext: false }),
    );
    // Wait on the percent — it needs the async byNode fetch to land (maxTokens
    // resolves synchronously from the graph config and would race ahead).
    await waitFor(() => {
      expect(result.current.selectedThreadHeaderContextPercent).toBeCloseTo(25);
    });
    expect(result.current.selectedThreadHeaderContextMaxTokens).toBe(200_000);
  });

  it('claude-agent: the maxContext flag selects the 1M window → 50K/1M = 5%', async () => {
    const { result } = renderForNode(
      makeGraph('claude-agent', { maxContext: true }),
    );
    await waitFor(() => {
      expect(result.current.selectedThreadHeaderContextPercent).toBeCloseTo(5);
    });
    expect(result.current.selectedThreadHeaderContextMaxTokens).toBe(1_000_000);
  });

  it('simple-agent: still reads summarizeMaxTokens (refactor regression guard)', async () => {
    const { result } = renderForNode(
      makeGraph('simple-agent', { summarizeMaxTokens: 100_000 }),
    );
    // 50000 / 100000 = 50%
    await waitFor(() => {
      expect(result.current.selectedThreadHeaderContextPercent).toBeCloseTo(50);
    });
    expect(result.current.selectedThreadHeaderContextMaxTokens).toBe(100_000);
  });

  it('other node types have no gauge → undefined percent', async () => {
    const { result } = renderForNode(makeGraph('runtime', {}));
    await waitFor(() => {
      expect(result.current.selectedThreadThreadUsage).toBeDefined();
    });
    expect(result.current.selectedThreadHeaderContextMaxTokens).toBeUndefined();
    expect(result.current.selectedThreadHeaderContextPercent).toBeUndefined();
  });
});
