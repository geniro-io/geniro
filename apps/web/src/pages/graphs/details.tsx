...
import {
  type ExecuteTriggerDto,
  type ExecuteTriggerDtoMessagesInner,
  type GraphDto,
  GraphDtoStatusEnum,
  type GraphNodeWithStatusDto,
  type GraphRevisionDto,
  GraphRevisionDtoStatusEnum,
  type StatusProviderEnum,
  type TemplateDto,
} from '../../autogenerated';
import {
  AlertDialog,
...
export const GraphPage = () => {
  ...
  // Per-provider OAuth auth-state for the node pre-flight warn. Provider ->
  // authenticated. A missing entry means "status not loaded yet" (the warn
  // stays quiet); an explicit `false` drives the unauthenticated warn.
  const [oauthStatusByProvider, setOAuthStatusByProvider] = useState<
    Record<string, boolean>
  >({});

  const refreshOAuthStatus = useCallback(async (providers: string[]) => {
    ...
    const entries = await Promise.all(
      providers.map(async (provider) => {
        try {
          const { data } = await oauthApi.status(
            provider as StatusProviderEnum,
          );
          return [provider, data.authenticated] as const;
        } catch (err) {
          console.warn(`Failed to fetch OAuth status for ${provider}:`, err);
          return null;
        }
      }),
    );
  ...
  // The unique OAuth providers referenced by the current graph's nodes. Keyed
  // to a stable string so dragging a node (which churns the `nodes` reference)
  // doesn't re-fetch when the provider set is unchanged.
  const oauthProviders = useMemo(
    () => GraphValidationService.collectOAuthProviders(nodes, templates),
    [nodes, templates],
  );
  const oauthProvidersKey = oauthProviders.join(',');

  useEffect(() => {
    if (!oauthProvidersKey) {
      return;
    }
    void refreshOAuthStatus(oauthProvidersKey.split(','));
  }, [oauthProvidersKey, refreshOAuthStatus]);

  // Re-fetch a provider's status the moment the user authenticates it inline
  // (the node Authenticate widget broadcasts success), so the warn clears live.
  useEffect(() => {
    ...
    channel.onmessage = (event: MessageEvent) => {
      if (isOAuthSuccessMessage(event.data)) {
        void refreshOAuthStatus([event.data.provider]);
      }
    };
    return () => {
      channel.close();
