...
A token field can carry a third marker, `x-ui:oauth-authenticate: { provider }`. Unlike the two `secret-select*` markers it is NOT a secret-resolution channel at all — it is a **UI-only affordance** that drives the per-MCP-node "Authenticate" widget (`apps/web/src/pages/oauth/RjsfOAuthAuthenticateField.tsx`). It carries no value and is read only by the front-end rjsf field; `collectSecretNames` matches solely `x-ui:secret-select` / `-multi-select`, so the oauth marker is invisible to the compiler (same exclusion mechanism that drops the host-only marker).
