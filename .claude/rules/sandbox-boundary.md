...
- **Re-throw the fail-closed exception at EVERY swallow point, not just the resolver.** A resolver that fails closed by throwing (e.g. `getValidatedPat` → `InternalException`) is defeated by a single downstream `catch` that degrades to an unauthenticated path. Every such catch MUST re-throw the fail-closed exception class (`if (error instanceof InternalException) throw error;`) while still letting the benign "no credential available" case (a plain `Error`) fall through to the unauthenticated/public path. Adding a new consumer that swallows token-resolution errors silently re-opens the anonymous-fallback hole — the resolver's throw is only as strong as the weakest catch between it and the operation. Exemplars: `gh-clone.tool.ts` `invoke` AND `gh-base.tool.ts` `execGhCommand` both re-throw the PAT-misconfig `InternalException` (`apps/api/src/v1/agent-tools/tools/common/github/`).
...
### `x-ui:oauth-authenticate` is a third, ORTHOGONAL marker — UI affordance, not a credential channel

A token field can carry a third marker, `x-ui:oauth-authenticate: { provider }`. Unlike the two `secret-select*` markers it is NOT a secret-resolution channel at all — it carries no value and NEVER routes a credential anywhere. It is read purely as an IDENTIFIER (which provider a node authenticates against) by three separate consumers: the per-MCP-node "Authenticate" widget (`apps/web/src/pages/oauth/RjsfOAuthAuthenticateField.tsx`); the M3.2 deploy pre-flight on the SERVER (`collectOAuthNodes` in `apps/api/src/v1/graphs/services/oauth-node.utils.ts`, walked by `GraphsService.run()`'s `assertOAuthCredentialsValid` gate); and the M3.2 editor warn on the CLIENT (`GraphValidationService.collectOAuthProviders` / `checkSecretReferences`). It remains invisible to the compiler's SECRET collection: `collectSecretNames` matches solely `x-ui:secret-select` / `-multi-select` (same exclusion mechanism that drops the host-only marker), so the oauth marker never reaches the runtime `secretEnv` — its readers use the provider string only to decide which stored credential to status-check, never to inject a value. The three reader walks mirror `collectSecretNames`' field-meta shape; keep them in lockstep to avoid drift.

Crucially, on the Linear MCP token field this marker is **expected to co-exist with `x-ui:secret-select`** on the SAME field (`linear-mcp.template.ts`): `secret-select` is what injects the OAuth token into the runtime env (the sandbox-cred channel), while `oauth-authenticate` only renders the connect button that acquires/rotates that token via the per-project OAuth flow. The two are independent — the token reaches the sandbox exactly as any other `secret-select` secret does, and the oauth marker must NEVER be the thing that routes a value anywhere (and must never be conflated with `x-ui:secret-select-host`, which sits on the opposite side of the boundary). The graph-compiler exclusion test pins that a field carrying `oauth-authenticate` is not collected by `collectSecretNames` even when it resolves to a non-undefined value.
